...
def norm_data(df):
    ...
    df_x = pd.DataFrame(x)
    df_r = pd.DataFrame(r)
    df2 = pd.concat([df_x, df_r], axis=1)
    df2.columns = cols
    return df2
...
def preprocess_data():
    ...
    df_train = norm_data(df_train)
    df_test = norm_data(df_test)
    # df_train

    df_train.to_csv("nslkdd-train-pro-norm.csv", index=False)
    df_test.to_csv("nslkdd-test-pro-norm.csv", index=False)
...
def get_datasets(csv_file, is_binary, test_ratio = 0.2):
    ...
    # train.to_csv("train-train-norm.csv", index=False)
    # test.to_csv("train-test-norm.csv", index=False)
    return (NSLKDDDataset(train, is_binary), NSLKDDDataset(test, is_binary))

# get_datasets("./nslkdd-train-pro-norm.csv", False, 0.1)